...
def load_and_combine_data():
    ...
    # 基礎清洗：排除沒有學院資料的課程
    df = df.dropna(subset=['主開學院名稱_中文'])
    df = df[df['主開學院名稱_中文'].str.strip() != '']
...
# --- 主程式 ---
try:
    data_res = load_and_combine_data()
    df, tags_list, college_list, dept_list, year_list = data_res
    ...
    if df is not None:
        ...
        # --- 新增功能：排除關鍵字 ---
        st.sidebar.write("##### 🚫 排除特定課程")
        # 讓使用者可以自行輸入或選擇要排除的關鍵字
        exclude_keywords = st.sidebar.multiselect(
            "選擇或輸入要排除的關鍵字：",
            options=["導師時間", "論文", "專題製作", "書報討論", "體育"],
            default=["導師時間", "論文"]
        )
        ...
        # 2. 排除關鍵字過濾 (重點邏輯)
        if exclude_keywords:
            # 建立一個正規表示式的字串，例如 "導師時間|論文"
            pattern = "|".join(exclude_keywords)
            # 排除掉名稱中包含這些字串的資料
            f_df = f_df[~f_df['主開科目名稱'].str.contains(pattern, na=False, case=False)]
        ...
        if not unique_courses_df.empty:
            ...
            with row2_col1:
                ...
                fig_col_h = px.bar(stats_college_bar, y='主開學院名稱_中文', x='課程數', orientation='h', text='課程數', color='課程數')
            ...
            with row2_col2:
                ...
                fig_dept_h = px.bar(stats_dept, x='課程數', y='主開系所名稱_中文', orientation='h', text='課程數')
        ...
        st.divider()
        st.subheader("📋 課程詳細清單")
        display_cols = ['學年度', '學期', '主開學院名稱_中文', '主開系所名稱_中文', '主開課程碼', '主開科目名稱', '學分數', '選別', '主授教師', '課程標籤']
        available_display_cols = [col for col in display_cols if col in unique_courses_df.columns]
        st.dataframe(unique_courses_df[available_display_cols].reset_index(drop=True), use_container_width=True)
